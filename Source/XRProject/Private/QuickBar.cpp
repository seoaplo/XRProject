...
void UQuickBar::SetQuickSlot()
{
	for (int i = 0; i < kQuickBarSize; i++)
	{
		...
		RefreshKeyName(i);
		list[i]->SlotIndex = i;
		switch (QuickBar::GetInstance().Data[i].Type)
		{
		case 0:
			break;
		case 1:
		{
			UPlayerSkill* Skill = XRGI->GetPlayerSkillManager()->FindSkillFromList(XRGI->GetPlayerSkillManager()->SkillListForPlalyer, QuickBar::GetInstance().Data[i].ID);
			list[i]->TargetObject = Skill;
			list[i]->bIsSkill = true;
			list[i]->ID = Skill->GetID();
			list[i]->SetIcon();
			break;
		}
		case 2:
		{
			UItem* Item = XRGI->ItemManager->GetItemFromId(EItemType::CONSUMPTION, QuickBar::GetInstance().Data[i].ID).GetValue();
			list[i]->TargetObject = Item; // 스킬 생성한 객체를 주는 걸로
			list[i]->bIsSkill = false;
			list[i]->ID = Item->GetID();
			list[i]->SetIcon();
			break;
		}
		default:
